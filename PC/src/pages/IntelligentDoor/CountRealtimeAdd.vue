<template>
  <div class="hello">
    <div class="listtitle" v-if="view === 'true'">查看实时计数任务</div>
    <div class="listtitle" v-if="view === undefined && ruleForm.taskId === undefined">新增实时计数任务</div>
    <div class="listtitle" v-if="view === undefined && ruleForm.taskId !== undefined">编辑实时计数任务</div>
    <el-form :model="ruleForm" :rules="rules" ref="ruleForm" label-position='left' label-width="100px" class="demo-ruleForm newaddInfoC">
      <el-form-item label="任务名称" prop="countingName">
        <el-input v-model="ruleForm.countingName" :disabled='view === "true" ? true : false'></el-input>
      </el-form-item>
      <el-form-item label="任务描述" prop="countingDesc">
        <el-input type="textarea" v-model="ruleForm.countingDesc" :disabled='view === "true" ? true : false'></el-input>
      </el-form-item>
      ...
      <!-- <el-form-item label="计数周天" prop="checkWeekList">
        <div>
      ...
      <el-form-item label="计数日期" prop="timerange">
        <el-date-picker
          v-model="ruleForm.countingStart_unform"
          type="date"
          :disabled='view === "true" ? true : false'
          placeholder="开始时间">
        </el-date-picker>
        ~
        <el-date-picker
          v-model="ruleForm.countingEnd_unform"
          type="date"
          :disabled='view === "true" ? true : false'
          placeholder="结束时间">
        </el-date-picker>
      </el-form-item>
      <el-form-item label="计数时间" prop="countingOptionEnd_unform">
        <el-time-picker
          v-model="ruleForm.countingOptionStart_unform"
          :picker-options="{
        ...
        ~
        <el-time-picker
          v-model="ruleForm.countingOptionEnd_unform"
          :picker-options="{
            selectableRange: '00:00:00 - 23:59:59'
          }"
          :disabled='view === "true" ? true : false'
      ...
      <el-form-item label="场景" prop="sendId">
        <el-select v-model="ruleForm.sendId" placeholder="请选择场景" :disabled='view === "true" ? true : false'>
      ...
      <el-form-item>
        ...
        <router-link class="el-button" to="/IntelligentDoor/CountRealtimeTask">取消</router-link>
      </el-form-item>
    </el-form>
...
export default {
  data () {
    ...
    const validateTimerange = (rule, value, callback) => {
        if (this.ruleForm.countingStart_unform === '' || this.ruleForm.countingEnd_unform === '') {
          callback(new Error('请选择计数日期'));
        }else if(this.ruleForm.countingStart_unform === '' && this.ruleForm.countingEnd_unform !== '' || this.ruleForm.countingStart_unform !== '' && this.ruleForm.countingEnd_unform === ''){
          callback(new Error('你必须同时选择开始和结束时间！'));
        }else if(this.ruleForm.countingStart_unform - this.ruleForm.countingEnd_unform >0){
    ...
    const validateDayTimerange = (rule, value, callback) => {
        if (this.ruleForm.countingOptionStart_unform === '' || this.ruleForm.countingOptionEnd_unform === '') {
          callback(new Error('请选择计数时间'));
        }else if(this.ruleForm.countingOptionStart_unform - this.ruleForm.countingOptionEnd_unform >0){
          callback(new Error('计数开始时间不能晚于结束时间！'));
        } else {
          callback();
        }
    };
    ...
    return {
      ...
      ruleForm: {
          ...
          countingWeek:'',
          countingStart:'',
          countingStart_unform:'',
          countingEnd:'',
          countingEnd_unform:'',
          countingOptionStart:'',
          countingOptionStart_unform:'',
          countingOptionEnd:'',
          countingOptionEnd_unform:'',
          sendId:'',
          diveodId:'',
          diveodIdList:[],
          personList:[],
      ...
      rules: {
          ...
          countingOptionEnd_unform:[
            ...
            { validator: validateDayTimerange, trigger: 'blur' },
          ],
          sendId: [
  ...
  mounted () {  
    ...
    self._ajax(self,'/get/door/getAll', {}, function (data) {
      ...
      if(self.ruleForm.taskId !== undefined){
        self._ajax(self,'/get/door/checkCountInfo', {'taskId':self.ruleForm.taskId}, function (data) {
          // self.grouplist = data.data;
          self.ruleForm.countingName = data.data.countingName;
          self.ruleForm.countingDesc = data.data.countingDesc;
          self.ruleForm.executionType = data.data.executionType;
          self.ruleForm.sendId= String(data.data.sendId)
          
          // self.ruleForm.checkWeekList=self.wekdaychangeRe(data.data.countingWeek.split(','));
          self.ruleForm.countingStart_unform = (new Date(data.data.countingStart));
          self.ruleForm.countingEnd_unform = (new Date(data.data.countingEnd));
          self.ruleForm.countingOptionStart_unform = (new Date(data.data.countingOptionStart));
          self.ruleForm.countingOptionEnd_unform = (new Date(data.data.countingOptionEnd));
  ...
  methods: {
    submitForm(formName) {
      ...
      // this.ruleForm.countingWeek = String(this.wekdaychange_t2n(this.ruleForm.checkWeekList));
      self.ruleForm.countingStart = self.formatTime(self.ruleForm.countingStart_unform,1);
      self.ruleForm.countingEnd = self.formatTime(self.ruleForm.countingEnd_unform,2);
      self.ruleForm.countingOptionStart = self.format_hms(self.ruleForm.countingOptionStart_unform);
      self.ruleForm.countingOptionEnd = self.format_hms(self.ruleForm.countingOptionEnd_unform)
      this._submitForm(formName,this,'/get/door/addCompactionmeter',this.ruleForm,'/IntelligentDoor/CountRealtimeTask')
    },
