  ...
  <div class="hello">
    ...
    <div class="listtableC">
      <el-table
        ...
        <el-table-column label="最后抓拍时间" width=230>
        ...
        <el-table-column label="状态">
          <template slot-scope="scope">
            <span v-if="scope.row.status === '0'">离岗</span>
...
export default {
  ...
  mounted () {  
    let self = this;
    // 任务
    self._ajax(self,'/get/door/getMakeTaskAll', {}, function (data) {self.attendanceName = data.data;})
    // 分组
  ...
  methods: {
    querylist(){
      let self = this;
      self._ajax(self,'/get/door/patrolRecordList', self.querydata, function (data) {
    ...
    exportEposure(){
      // 导出
      var form = $('#searchForm');
      form.attr("action", this.APIURL + "/get/door/exportMakeRecord");
      form.submit();
    },
    patrolNow(){
      // 立即巡查
      let self = this;
      if(self.querydata.taskId !== ''){
        self.$confirm('是否立即巡查?', '提示', {
          confirmButtonText: '确定',
          cancelButtonText: '取消',
          type: 'warning'
        }).then(() => {
          self._ajax(self,"/get/door/immediateInspection",{'taskId':self.querydata.taskId}, function (data) {
            self.$message({
              type: 'success',
              message: data.desc
            });
