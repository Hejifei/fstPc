  ...
  <div class="hello">
    ...
    <div class="listSearchC">
      <el-input
        ...
        v-model="querydata.exposureName"
        clearable>
      </el-input>
      <el-button round @click="querylist">查询</el-button>
      <el-row>
        <el-col :span="12">
          <router-link to="/IntelligentDoor/ExposureTimesAdd" class="el-button">新增</router-link>
    ...
    <div class="listtableC">
      <el-table
        ...
        <el-table-column label="任务名称">
          <template slot-scope="scope">{{ifnull(scope.row.exposureName)}}</template>
        </el-table-column>
        ...
        <el-table-column label="曝光时间" width=120>
        ...
        <el-table-column label="操作" width=240>
          <template slot-scope="scope">
            <router-link :to="'/IntelligentDoor/ExposureTimesAdd?id='+scope.row.id+'&&view=true'" class="el-button el-button--mini redbtn">查看</router-link>
            ...
            <router-link v-if="scope.row.status === '2'" :to="'/IntelligentDoor/ExposureTimesAdd?id='+scope.row.id" class="el-button el-button--mini redbtn">编辑</router-link>
...
export default {
  ...
  methods: {
    querylist(){
      let self = this;
      self._ajax(self,'/get/door/exposureList', self.querydata, function (data) {
    ...
    taskStop(id){
      //暂停
      this.task_Stop(this,'/get/door/updateExposureStatus',{'taskId':id})
    },
    taskStart(id){
      //启动
      this.task_Start(this,'/get/door/attendanceStart',{'taskId':id})
    },
    ...
    listDelete(){
      // 列表页删除
      this.Delete(this,'/get/door/deleteExposure',{'taskId':String(this.delIds)});
    }
  },
}
</script>
