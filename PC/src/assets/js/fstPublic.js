...
function install(Vue, options) {
    ...
    // let APIURL = 'http://172.16.0.242:8080'
    // let APIURL = 'http://221.224.53.27:10019' //外网映射
    // let APIURL = 'http://221.224.99.248:13306'
    let APIURL = 'http://10.10.30.137:13306' //20181009打包api
    ...
    const _ajax = function(that,_url, a, b, c ) {
        ...
        if (typeof c != "function") {
            c = function (data) {
                that.$message.error(data.desc)
            }
        }
        let headers={};
        let xtoken ='';
        // 除了登录接口，别的都要传header
        if(_url !== '/get/system/login'){
            xtoken = JSON.parse(localStorage.fst).token;
    ...
    const _fileajax = function(that,_url, a, b, c ) {
        const loading = that.$loading({
            lock: true,
            text: 'Loading',
            spinner: 'el-icon-loading',
    ...
    //公共的添加方法
    //formName  that:this apiurl:接口名 urlRdt:提交成功重定向界面
    const _submitForm = function(formName,that,apiurl,formdata,urlRdt) {
        that.$refs[formName].validate((valid) => {
            if (valid) {
                _ajax(that,apiurl, formdata, function (data) {
                    that.$message({
                        message: data.desc,
                        type: 'success'
                    });
                    if(urlRdt !== ''){
                        that.$router.push({path:urlRdt});
                    }
    ...
    // 暂停的公共方法
    const task_Stop=(that,url,id)=>{
        //暂停
        that.$confirm('是否暂停?', '提示', {
          confirmButtonText: '确定',
          cancelButtonText: '取消',
          type: 'warning'
        }).then(() => {
    ...
    // 启动公共方法
    const task_Start=(that,url,id)=>{
        // 启动
        that.$confirm('是否启动?', '提示', {
          confirmButtonText: '确定',
          cancelButtonText: '取消',
          type: 'warning'
        }).then(() => {
    ...
    //列表页操作提示
    //that-this url-接口 dataSubmit-提交的数据 titleTxt-提示语
    const listOperation = (that,url,dataSubmit,titleTxt)=>{
        if(dataSubmit.id.length === 0){
            that.$message({
                type: 'warning',
                message: '您还没有选择要同步的设备！'
            });
        }else{
            that.$confirm(titleTxt, '提示', {
                confirmButtonText: '确定',
                cancelButtonText: '取消',
                type: 'warning'
            }).then(() => {
                _ajax(that,url, dataSubmit, function (data) {
                    that.$message({
                        type: 'success',
                        message: data.desc
                    });
    ...
    const format_hms = (time)=>{
        if(time === '' || time === null ){
            ...
            let nowTime = size2(h) + ':' + size2(m) + ':' + size2(s);
            return nowTime;
        }
    ...
    const Format_ymd = function (timestamp ) {
        if(timestamp !== null){
            var date = new Date(timestamp);//时间戳为10位需*1000，时间戳为13位的话不需乘1000
            let Y = date.getFullYear();
            let M = date.getMonth()+1 ;
            let D = date.getDate();
            return Y+'-'+size2(M)+'-'+size2(D);
        }else{
            return null;
        }
        
    }
    const FormatTable = function (timestamp ) {
        var date = new Date(timestamp);//时间戳为10位需*1000，时间戳为13位的话不需乘1000
        let Y = date.getFullYear() + '-';
        let M = (date.getMonth()+1 < 10 ? '0'+(date.getMonth()+1) : date.getMonth()+1) + '-';
        let D = date.getDate()<10?'0'+date.getDate():date.getDate();
        let h = ' '+ size2(date.getHours()) + ':';
        let m = date.getMinutes();
        return Y+M+D+h+size2(m);
    }

    const getYear = (timeinsert)=>{
        var date = new Date(timeinsert)
        let Y =date.getFullYear();
        return Y;
    }
    ...
    function createTable(options, e) {
        ...
        if (options.week) {
            ...
            setconfigDay(options, datetable.count, options.datelist);
        }
    ...
    function contextmenuClick(options, count, e) {
        ...
        $("#menu-overtime").click(function () {
            ...
            configDayM["D" + D] = 0;
            options.calRest(`${Y}-${size2(M)}-${size2(D)}`)
        ...
        $("#menu-cancel").click(function () {
            ...
            options.calnoRest(`${Y}-${size2(M)}-${size2(D)}`)
            $(".dayRightClick").remove();
        });
        // 返回
    ...
    function btnClick(options, e, count) {
        ...
        $(".days").click(function () {
            ...
            if($(this).hasClass('cal_rest')){
                options.calnoRest(`${Y}-${size2(M)}-${size2(D)}`)
                $(this).removeClass('cal_rest')
            }else{
                options.calRest(`${Y}-${size2(M)}-${size2(D)}`)
                $(this).addClass('cal_rest')
            }
    ...
    const wekdaychange = (weklistarr)=>{
        if(weklistarr.length === 0){
            return ''
        }else{
            let wektxtlist = ['星期日','星期一','星期二','星期三','星期四','星期五','星期六',]
            let weklist = weklistarr.split(',');
            let newttext ='';
            for(let i in weklist){
                newttext += wektxtlist[(weklist[i]-1)]+'、';
            }
            return newttext;
        }
    }
    const wekdaychangeRe = (weklistarr,str)=>{
        if(typeof(weklistarr) === 'string'){
            weklistarr = weklistarr.split(',')
        }
        if(weklistarr.length === 0){
            return ''
        }else{
            ...
            // let weklist = ;
            let newttext =[];
            for(let i in weklistarr){
                newttext.push(wektxtlist[(weklistarr[i]-1)]);
            }
            if(str === undefined){
                return newttext;
            }else{
                return String(newttext);
            }
            
        }
    }
    const wekdaychange_t2n = (weklistarr)=>{
        if(weklistarr.length === 0){
            return ''
        }else{
            let wektxtlist = ['星期日','星期一','星期二','星期三','星期四','星期五','星期六',]
            let newttext =[];
            for(let i in weklistarr){
                newttext.push(wektxtlist.indexOf(weklistarr[i])+1);
            }
            return newttext;
        }
    }
    // 执行时间 时间拼接成string
    const execution_timt2str=(timeobs)=>{
        // 任务时间数据拼接
      let timeList = [];
      for(let i in timeobs){
        if(i !== 'year'){
          timeList.push(timeobs[i] === '' ? '*' : timeobs[i])
        }
      }
      // 将选择的年份排序
      let year = timeobs.year;
    ...
    Vue.prototype._fileajax=_fileajax;
    Vue.prototype._submitForm=_submitForm;
    Vue.prototype.Delete = Delete;
    Vue.prototype.task_Stop=task_Stop;
    Vue.prototype.task_Start=task_Start;
    Vue.prototype.listOperation = listOperation;
    Vue.prototype.getUrl=getUrl;
    Vue.prototype.settime=settime;
    Vue.prototype.unixChange=unixChange;
    Vue.prototype.formatTime=formatTime;
    Vue.prototype.format_hms=format_hms;
    Vue.prototype.Format=Format;
    Vue.prototype.Format_ymd=Format_ymd;
    Vue.prototype.FormatTable=FormatTable;
    Vue.prototype.getYear=getYear;
    Vue.prototype.size2=size2;
    Vue.prototype.createcal=$.fn.calendar;
    Vue.prototype.ifnull = ifnull;
    Vue.prototype.wekdaychange=wekdaychange;
    Vue.prototype.wekdaychangeRe=wekdaychangeRe;
    Vue.prototype.wekdaychange_t2n= wekdaychange_t2n;
    Vue.prototype.execution_timt2str=execution_timt2str;
